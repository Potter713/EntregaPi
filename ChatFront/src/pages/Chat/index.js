...
import {
  Container,
  Header, HeaderContent, HeaderPhoto, PhotoHeader, BoxTexts, HeaderName, HeaderDescription, HeaderButtons, HeaderButton,
...
import fundoChat from './../../assets/img/fundoChat.jpg'
...
export default function Chat({ history }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      response.data.patients[0]._id !== patient_id 
      ?
        contactSelected(response.data.patients[0])
      :
        contactSelected(response.data.patients[1])
    }
    fetchData();
  ...
  const socket = useMemo(() => socketio('https://psicotecchatback.herokuapp.com/', {
    query: { patient_id }
  }), [patient_id]);
  ...
  function readKey(e) {
    ...
    if (e.keyCode === 38) {
      if(messageInput !== "" && messageInput !== prevMessage){
        setNextMessage(messageInput);
      }
  ...
  return (
    ...
    <Container contactsOpen={contactsOpen} emojisOpen={boxEmj} style={{ background: `url("${fundoChat}")` }}>
      <Header contactsOpen={contactsOpen}>
        ...
        <HeaderButtons>
          <HeaderButton onClick={goContacts}>
            Voltar
          </HeaderButton>
        </HeaderButtons>
      ...
      <Aside contactsOpen={contactsOpen}>
      ...
      <Main contactsOpen={contactsOpen}>
        {
          messages.map(msg => (
      ...
      <BoxSend contactsOpen={contactsOpen}>
